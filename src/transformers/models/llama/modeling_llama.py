...
class SepEmbLlamaAttention(nn.Module):
    ...
    def forward(
        ...
        ## Gets past KV cache length and adds it to effective seq len
        kv_seq_len = key_states.shape[-2]
        if past_key_value is not None:
            kv_seq_len += past_key_value[0].shape[-2]
        
        ## Concats old KV to current
        if past_key_value is not None:
            # reuse k, v, self_attention
            key_states = torch.cat([past_key_value[0], key_states], dim=2)
            value_states = torch.cat([past_key_value[1], value_states], dim=2)
        # KV is now full length, Q is still 1 token
...
class MultimodalRPEAttention(nn.Module):
    """
    Vanilla attention with KV cache support. Applies RPEs after loading from
    KV cache as opposed to applying RPEs and saving to KV cache. Helps to better
    handle position embeddings for different 
    """
    def __init__(self, config: LlamaConfig):
        super().__init__()
        self.config = config
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig):
        ...
        # self.self_attn = SepEmbLlamaAttention(config=config)
        self.self_attn = MultimodalRPEAttention(config=config)
...
class Deprecated_SepEmbLlamaAttention(nn.Module):
    """Multi-headed attention from 'Attention Is All You Need' paper""" 
    ...
    def _calculate_modality_masks(self, attn_weights):
        ...
        image_attn_mask = torch.zeros_like(attn_weights)
        video_attn_mask = torch.zeros_like(attn_weights)
        text_attn_mask = torch.zeros_like(attn_weights)
        ...
        modalities_buffer = Deprecated_SepEmbLlamaAttention.inputs_emb_modalities
        # List[List[Dict['modality': num_tokens]]]
